...
[Getting started](getting-started.ipynb) page).
...
[**Skip edges**](skip-edges.ipynb) for residual wiring.
...
`forward()`. See [**Skip edges**](skip-edges.ipynb).

Cycles are not parsed in v1. A recurrent (fixed-step) update is
ordinary user `forward()`. See the
[**Recurrent example**](recurrent-example.ipynb).

See [**Mapping attributions**](map-node-attributions.ipynb) for
how `map_node_attributions()` labels a layer tensor.
...
- [**Getting started**](getting-started.ipynb) for a full
  end-to-end feedforward example
- [**Recurrent example**](recurrent-example.ipynb) for a cyclic
  graph with a shared masked update
- [**Mapping attributions**](map-node-attributions.ipynb) for
  labeling layer tensors with node names
- [**Skip edges**](skip-edges.ipynb) for keeping activations in
  memory instead of pseudo nodes
- [**API reference**](api.md) for function- and object-level
  documentation
