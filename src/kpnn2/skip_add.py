...
class SkipAdd(nn.Module):
    ...
    def forward(
        ...
        n_layers = len(self.spec.layer_nodes)
        ...
        n_units = len(self.spec.layer_nodes[target_layer])
        if hidden.ndim < 1:
            raise Kpnn2Error(
        ...
        for index, skip in enumerate(self.spec.skips):
            ...
            n_source = len(self.spec.layer_nodes[skip.source_layer])
            ...
            source = source_tensor[..., skip.source_index]
            ...
            term[..., skip.target_index] = weight * source
            out = out + term
        return out
